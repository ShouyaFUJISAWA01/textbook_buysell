...
#選択された教科書の情報変更ページを表示
@book_management.route('/book_info_change/show')
def book_info_show(id):
    book_info=Book.query.get(id)
    return render_template('book_info_change_admin.html', book_info=book_info)
...
def book_search():
    ...
    if searched_title == '':
        book_info = Book.query.all()
    else:
        book_info = Book.query.filter(Book.title==searched_title)
    return render_template('book_management.html', book_info=book_info)

#選択された教科書を削除し、教科書一覧を再表示する
@book_management.route('/book_info_change/delete', methods=['POST'])
def book_delete(id):
    book_info = Book.query.get(id)
    db.session.delete(book_info)
    db.session.commit()
    return redirect(url_for('book_management.book_search'))

#選択された教科書内容を入力内容に更新し、一覧を再表示する
@book_management.route('/book_info_change/update', methods=['POST'])
def book_update(id):
    book_info = Book.query.get(id)
    book_info.isbn_no = request.form.get('isbn_no')
